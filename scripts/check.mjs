...
for(const id of order){click(`[data-unit="${id}"]`);assert.equal(w.document.querySelector('main').dataset.screen,'locks');assert(w.document.querySelector('.lock-status-menu').getAttribute('aria-label').includes(app._locks[id].label));for(const el of w.document.querySelectorAll('[data-status]'))assert.equal(el.getAttribute('aria-label'),app._statusLabel(el.dataset.status),'Original status meanings retained');click('[data-choice-close]');}
...
for(const st of ['all','action','green','red','black','purple','blue','yellow','white','flashred','flashgreen']){
 click('#tablet-filter');click(`[data-filter-status="${st}"]`);assert(!w.document.querySelector('.lock-filter-menu'),'Choosing a filter closes it');
...
click('[data-nav="locks"]');click('#tablet-filter');click('[data-filter-status="all"]');
...
click('[data-history-order="oldest"]');assert(w.document.querySelector('.history-row').textContent.includes('C2'),'Oldest saved change is reachable');
assert.deepEqual([...w.document.querySelector('.history-row').querySelectorAll('.lock-disc')].map(d=>d.dataset.lockStatus),['green','red'],'History shows the saved before and after colors');assert.equal(w.document.querySelector('.history-row').dataset.historyRecord,'early','The original history ID remains attached to its displayed record');assert.equal(w.document.querySelectorAll('.lock-color-key').length,1,'One shared key across tablet screens');
click('[data-history-type="all"]');assert.equal(w.document.querySelectorAll('.history-row').length,612,'Email records also remain in the full archive');assert(w.document.querySelector('[data-history-backup]').download.startsWith('storewell-complete-history-'));
