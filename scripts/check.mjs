...
w.document.dispatchEvent(new w.KeyboardEvent('keydown',{key:'w',bubbles:true}));await until(()=>Number(deck.dataset.bridgeZ)<3.7,8000);await until(()=>Number(deck.dataset.bridgeY)>5.4);await new Promise(r=>setTimeout(r,300));w.document.dispatchEvent(new w.KeyboardEvent('keyup',{key:'w',bubbles:true}));
...
assert.equal(w.document.querySelector('#sw-tab-activity').style.display,'block','Gear opens the retained Activity log');
click('#sw-panel-close');click('[data-action="gear"]');click('[data-setting="contacts"]');
await new Promise(resolve=>setTimeout(resolve,20));
assert.equal(w.document.querySelector('#sw-tab-control').style.display,'block','Gear opens the retained Control tools');
...
click('[data-tab="action"]');assert(w.document.querySelector('#sw-action-list [data-lock-unit]'),'Action Required lists locks that need action');
let operationsSave;app.setStatus=async(label,state)=>{operationsSave={label,state};app._overrides[label.replace(/[-\\s]/g,'').toUpperCase()]=state;return true;};
click('#sw-action-list [data-lock-status="red"]');await new Promise(resolve=>setTimeout(resolve,20));
assert.equal(operationsSave.state,'red','Action Required Lock writes through the shared status save path');
click('[data-tab="all"]');const lockSearch=w.document.querySelector('#sw-lock-search');lockSearch.value='G2';lockSearch.dispatchEvent(new w.Event('input'));
assert(w.document.querySelector('#sw-all-list').textContent.includes('G2'),'All Locks search filters by lock label');
click('#sw-all-list [data-lock-status="green"]');await new Promise(resolve=>setTimeout(resolve,20));
assert.equal(operationsSave.state,'green','All Locks Open writes through the shared status save path');
app.setStatus=savedStatusFn;
